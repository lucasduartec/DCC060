...
SELECT
	i.nome_item,
SUM
	(contem.quantidade) AS total_pedidos
FROM
	item i
INNER JOIN
	pedido_contem_item contem ON i.id_item = contem.id_item
GROUP BY
	i.nome_item
ORDER BY 
	total_pedidos DESC;
...
COUNT
	(p.id_pedido) AS total_pedidos_atendidos
FROM 
	pedido p
INNER JOIN 
...
SELECT 
	m.numero_mesa,
COUNT
	(p.id_pedido) AS total_pedidos_na_mesa
FROM 
	pedido p
INNER JOIN 
...
SELECT 
	i.categoria_item,
SUM 
	(contem.quantidade) AS total_itens_vendidos
FROM 
	item i
INNER JOIN 
	pedido_contem_item contem ON i.id_item = contem.id_item
